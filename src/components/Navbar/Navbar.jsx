...
const Navbar = () => {
  ...
  const [navZIndex, setNavZIndex] = useState(0); // Initial z-index

  const handleHamburger = () => {
    setmobilenav(!mobilenav);
    setNavZIndex(mobilenav ? 0 : 4); // Increase z-index when menu is open
  };

  const changezIndex = () => {
    if (document.getElementById('nav').style.zIndex !== 10) {
      console.log('10');
      document.getElementById('nav').style.zIndex = 10;
    } else if (document.getElementById('nav').style.zIndex === 10) {
  ...
  return (
    <div id="nav" className="w-screen h-[60px] flex items-center justify-between py-3 fixed top-0 px-5 ">
      <button
        ...
        className="text-white text-3xl absolute right-0 top-0 bg-black block md:hidden mr-3 shadow-md"
      >
      ...
      >
        ...
        <a
          ...
          class="group text-white transition-all duration-300 ease-in-out md:ml-8"
          href="#about"
        >
